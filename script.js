...
function drawNextFrame() {
    ...
    if (player.isRolling) {
        ...
        player.frameCount++;
        player.currentFrame = runningFrames[Math.floor(player.frameCount / player.frameDuration) % runningFrames.length];
    } else {
        player.currentFrame = idleFrame;
        player.rotation = 0; // Reset rotation when not rolling
    }
