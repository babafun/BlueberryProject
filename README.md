# Blueberry Project

Welcome to the Blueberry Project! This project was meant to be part of the QE Hackathon (which didn't happen...they promised us) and aims to provide an open-source library allowing programmers to easily create an innovative gaming experience using basic technology.

## Project Overview

The Blueberry Project focuses on developing a template so that devs can make simple 2D platformers on-the-fly. Our goal is to create a user-friendly interface that allows players to interact with the game in a meaningful way.

## License
All the art is free to use under CC BY-SA 4.0 (except for the fallback.png, which is generated by AI).
For more details, please check [the art license and credits](LICENSE_ART.md).

## Features we're planning

- **Data Visualisation**: Interactive charts and graphs to help players understand game statistics.
- **Leaderboards**: Real-time leaderboards to track player rankings and achievements.
- **Machine Learning Models**: Adaptive gameplay that evolves based on player behavior.
- **User Management**: Secure authentication and player profile management.

## Getting Started

To get started with the Blueberry Project, follow these steps:

1. Clone the repository: `git clone https://github.com/babafun/BlueberryProject.git`
2. Navigate to the project directory: `cd BlueberryProject`
3. Start the development server: `npm start`

Or use GitHub Desktop (much easier, doesn't require Node.js npm)!

## Contributing

We welcome contributions from the community. If you would like to contribute, please fork the repository and submit a pull request. Make sure to follow our [contribution guidelines](CONTRIBUTING.md).

## Play

To play the Blueberry Project game, follow these steps:

1. Ensure you have Python 3 installed on your machine. You can download it from [python.org][python].
2. Find the folder in this repository called ".start".
3. Run the respective "start-server" file in the ".start" folder (for Windows, that's `start-server.bat`, not `local-server-manager-windows.ps1`). [(why?)][explanation]
It should automatically open the game on your default browser. Enjoy your gaming experience!

Or just play online at [babafun.github.io/BlueberryProject/](https://babafun.github.io/BlueberryProject/).

Thank you for being a part of the Blueberry Project!

[python]: https://www.python.org/
[explanation]: WINDOWS_PS1_USAGE_EXPLANATION.md
